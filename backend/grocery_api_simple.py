...
def search():
    try:
        ...
        try:
            # Use the exact Atlas Search aggregation pipeline as shown in the screenshot
            pipeline = [
                {
                    "$search": {
                        "index": "default",
...
# Compatibility endpoint for products API
@app.route("/dataset/products")
@app.route("/api/dataset/products")
def get_products():
    # Just redirect to grocery products endpoint
