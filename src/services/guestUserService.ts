...
// Set guest user name in sessionStorage
export const setSessionGuestName = (name: string): void => {
  try {
    sessionStorage.setItem('guestUserName', name);
  } catch (error) {
    console.error('Error saving guest name to session storage:', error);
  }
};
...
export const getSessionGuestName = (): string => {
  try {
    return sessionStorage.getItem('guestUserName') || '';
  } catch (error) {
    console.error('Error retrieving guest name from session storage:', error);
    return '';
  }
};
...
// Get guest user name (now reads from session storage)
export const getGuestName = (): string => {
  return getSessionGuestName();
};
